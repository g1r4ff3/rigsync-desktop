...
describe('checkNvidiaDriverMismatch', () => {
  it('is not applicable (skip) on a machine with no NVIDIA GPU at all', async () => {
    const result = await checkNvidiaDriverMismatch(fakeProvider(null, []))
    expect(result.applicable).toBe(false)
  })

  it('matches when NVRM and the userspace package version are identical', async () => {
    const result = await checkNvidiaDriverMismatch(
  ...
  it('matches even when dpkg reports a Debian revision suffix (real dpkg -l format)', async () => {
  ...
  it("flags a mismatch and warns when versions differ (this machine's real state)", async () => {
  ...
  it('picks the userspace package whose major version matches NVRM, not just the first one', async () => {
    const result = await checkNvidiaDriverMismatch(
  ...
  it('warns when the kernel module is loaded but no matching dpkg package is found', async () => {
    const result = await checkNvidiaDriverMismatch(fakeProvider('580.159.03', []))
    expect(result.applicable).toBe(true)
    expect(result.matched).toBe(false)
    expect(result.warning).toContain('찾을 수 없습니다')
  })

  it('warns when a driver package is installed but the kernel module is not loaded', async () => {
    const result = await checkNvidiaDriverMismatch(
