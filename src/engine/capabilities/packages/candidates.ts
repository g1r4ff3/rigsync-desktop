...
export async function buildPackageSyncGroups(
  ...
  if (providers.apt.isAvailable()) {
    ...
    if (names.length > 0) {
      ...
      // 구현은 이 목록을 소리 없이 삼켰다).
      const classification = await classifyAptPackagesCached(providers.apt, names)
      // R6 R2: 이름 전부를 한 번에 배치 조회한다(159개도 프로세스 1회).
      const descriptions = await providers.apt.descriptions(names)
  ...
  if (providers.snap.isAvailable()) {
    ...
    const liveSet = new Set((await providers.snap.list()).map((r) => r.name))
    const names = [...new Set([...managedSet, ...liveSet])].sort()
    if (names.length > 0) {
      groups.push({
        capability: 'snap',
  ...
  if (providers.flatpak.isAvailable()) {
    ...
    if (names.length > 0) {
      ...
      const details = await providers.flatpak.appDetails()
      groups.push({
        capability: 'flatpak',
        title: 'flatpak',
        items: names.map((name) => {
