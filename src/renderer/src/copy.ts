...
export const helpCopy = {
  ...
  doctor: [
    ...
    'Apt PATH shadowing은 관리 중인 apt 패키지와 같은 이름의 실행파일이 PATH에서 dpkg 관리본보다 먼저 잡히는지 확인합니다(예: /usr/local/bin이 /usr/bin을 가림) — rclone 공식 설치 스크립트가 깐 최신 버전이 apt의 구버전에 고지 없이 덮일 뻔한 실사고 재발 방지입니다. 두 가지 한계가 있습니다: 패키지명과 실행파일명이 다르면(예: ripgrep의 실행파일은 rg) 잡아내지 못하고, PATH는 rigsync 앱 프로세스가 보는 PATH를 기준으로 하므로 로그인 셸이 .bashrc 등에서 PATH를 다르게 구성했다면 실제와 다를 수 있습니다. 해소 명령은 표시만 됩니다 — rigsync는 어떤 파일도 자동으로 지우지 않습니다.',
    'Apt no repository candidate는 관리 중인 apt 패키지 중 apt-cache policy가 후보 버전을 전혀 못 찾는 것이 있는지 확인합니다 — 저장소 없는 로컬 .deb(공식 설치 프로그램 등)가 apt 관리 목록에 잘못 들어간 경우가 전형입니다. 이 상태로는 follower의 Apply가 "Unable to locate package"로 확실히 실패하므로, 발견되면 Flatpak이나 binaries capability로 라우팅을 다시 검토하세요(docs/package-policy.md).',
    'Manifest local edits는 manifest 작업 트리가 커밋되지 않은 채로 바뀌어 있는지 봅니다 — follower의 홈 파일은 manifest로의 심링크라 로컬 편집이 다음 pull을 실패시켜 동기화를 정지시키므로 경고로 표시되고, reference의 라이브 편집은 다음 드리프트 주기에 자동으로 별도 커밋(live-edit)으로 정리되므로 중립 정보로만 표시됩니다.',
